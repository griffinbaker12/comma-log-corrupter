    ...
    if not all(os.path.isfile(os.path.join(SCHEMA_BASE, schema)) for schema in schemas):
        ...
        for schema in schemas:
            response = requests.get(
                f"https://raw.githubusercontent.com/commaai/openpilot/master/cereal/{schema}".replace(
                    "+", "%2B"
                )
            )
...
def verify(qlogs, qcams):
    ...
    for index, qlog in enumerate(qlogs):
        ...
        checks = {
            "start segment": lambda: index > 0
            or any(
                msg.which() == "qRoadEncodeIdx" and msg.qRoadEncodeIdx.segmentNum == 0
                for msg in messages
            ),
            "all 60 qlogs": lambda: sum(
                1 for msg in messages if msg.which() == "qRoadEncodeIdx"
            )
            / 60
            == FPS,
            "all 60 GPS coordinates": lambda: sum(
                1 for msg in messages if msg.which() == "gpsLocation"
            )
            == 60,
        ...
        for m in messages:
            m_type = m.which()
            m_types[m_type] = m_types.get(m_type, 0) + 1

        for t, c in m_types.items():
            print(f"{t}: {c}")


class Downloader:
    def __init__(self, account=ACCOUNT, dongleId=DEVICE, route=ROUTE):
        self.files = self._request(
            f"https://api.commadotai.com/v1/route/{dongleId}|{route}/files",
            {"Authorization": f"JWT {account}"},
        )
        self.route = route
    ...
    def _request(self, url, headers):
        ...
        raise Exception(
            f"{url} returned a response {response.text} with status code {response.status_code}"
        )
    ...
    def download_resources(
        self, resources: "what all you want to download (eg, qlogs, qcams)"
    ):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for resource in resources:
                ...
                for index, url in enumerate(urls):
                    futures.append(
                        executor.submit(
                            _download, url, index, resource["name"], resource["ext"]
                        )
                    )
...
class Corrupter:
    ...
    def _corrupt_log(self, source, index, miss=[]):
        ...
        data = b"".join(
            ...
            for builder in [reader.as_builder()]
        )
    ...
    def corrupt(self, miss=[]):
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(
                    self._corrupt_segment, segment, self._qcams[index], index, miss
                )
...
def get_cli_args():
    parser = argparse.ArgumentParser(
        description="A script to corrupt openpilot routes for comma connect"
    )
    parser.add_argument(
        "-a", "--account", type=str, help="JWT access token for comma account"
    )
    parser.add_argument(
        "-d",
        "--device",
        type=str,
        help="dongleId of the device from which route was uploaded",
    )
    parser.add_argument(
        "-r", "--route", type=str, help="Unique ID of the route to be corrupted"
    )
...
def get_params():
    ...
    return [
        CAPNP_MAP[key]
        for key in Check(check="✅", choices=[key for key in CAPNP_MAP.keys()]).launch()
    ]
...
def main():
    ...
    downloader = (
        Downloader(account=account, dongleId=device, route=route)
        if account and device and route
        else Downloader()
    )
    paths = downloader.download_resources(
        [{"name": "qlogs", "ext": ".bz2"}, {"name": "qcameras", "ext": ".ts"}]
    )
